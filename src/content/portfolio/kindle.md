---
title: "Hacking your Kindle to display upcoming bus times"
description: ""
draft: true
tags: [hardware, software]
---

# How to hack an old Kindle to display upcoming bus times

I had an old Kindle (Kindle Touch 4th Generation aka "K5" aka "KT") lying around. This is how I turned it into a live bus feed that refreshes every minute with the option to exit out of "dashboard" mode by pressing the menu button. At a high level, the steps are:
...
I have a background in software so step 4 was the 'easiest' part for me but the rest of this process was my first time doing anything resembling jailbreaking/working with hardware. The rest was not easy and at times a frustrating exercise. Here's how I got to the end product and what I wished somebody had told me ahead of time.

## Jailbreaking your Kindle

This was my [Kindle hacking bible](https://www.mobileread.com/forums/showthread.php?t=225030) for steps 1 - 3. The first thing you need to do is figure out what version of Kindle you have, what firmware it's running (shorthand FW in these Kindle forum guides + readmes), download the appropriate tar file and follow installation instructions.

Once you've successfully jailbroken your Kindle, it's time to install some things.

## Installing KUAL & MRPI

KUAL is a custom Kindle app launcher. MRPI is what I needed to use to install custom apps onto my Kindle. You might not need MRPI if you have a newer version of Kindle. This part was very difficult for me, reading through the mobile forum threads gave me a headache. The most helpful resource I found for a n00b such as myself was the [Kindle modding wiki](https://kindlemodding.org/jailbreaking/post-jailbreak/installing-kual-mrpi/). Maybe other people aren't as oblivious as me but it took me way too long to realize that the "next step" in each individual guide can be accessed by clicking the "Next Step" button at the bottom of the page.

Another gotcha was that I _had_ to follow the [Setting up a Hotfix](https://kindlemodding.org/jailbreaking/post-jailbreak/setting-up-a-hotfix/) _first_ before attempting to install KUAL & MRPI. Otherwise, the KUAL/MRPI install would fail.
...
## Setup SSH for your Kindle

This can be done with a KUAL extension called USBNetwork (which you can also download from the [Kindle hacking bible](https://www.mobileread.com/forums/showthread.php?t=225030)) that will allow you to SSH onto your Kindle as if it were a regular server. However, nowhere in the forums could I find any information about how to actually install a KUAL extension using MRPI. Finally, this helpful [blogpost on setting up SSH for Kindle](https://blog.znjoa.com/2023/07/26/installing-usbnetwork-on-kindle/) came to my rescue. I followed the step that explains to how to install the extension, and how to setup SSH via USB. I ignored the rest of the instructions on the page because I'm not concerned about adding a password to the Kindle or setting up SSH over wifi.
...
## Running a server that generates an image for the Kindle

How displaying custom data on the Kindle works is that we first need to create a png that conforms to the Kindle screen size, then in the next step we'll be drawing that image onto the Kindle itself.

Since I live in New Jersey, I wanted to display NJTransit bus times on my Kindle. Luckily, NJTransit has a completely public GraphQL server that returns bus information for a specified stop number.

Feel free to skip this next section if you don't care about how to pull data from NJTransit. By inspecting the network tab of the [NJ Transit Bus Website](https://www.njtransit.com/bus-to) I was able to figure out the appropriate GraphQL Payload that returns the bus number, arrival time, current capacity, destination, and departing time in minutes. The query looks like this:
...
## Creating a KUAL dashboard extension
