---
title: "search board game rulebooks"
description: "Search for board game rulebooks, quickly jump to sections with keywords"
heroImage: "https://assets.mariannefeng.com/portfolio/rule-book.mp4"
heroAlt: "Video searching for a keyword in a board game rulebook, then searching for a particular board game rulebook. The settings modal is then toggled allowing the user to configure theme color and default rulebook language"
url: "https://rule-book.org"
pubDate: "Dec 10 2025"
---

#### what does it do

search for board game rulebooks by game then, then find relevant sections by searching for keywords

#### why

easier to share rulebooks while playing board games

#### how

a PWA with a <a href="https://github.com/mariannefeng/rulebook" target="_blank" rel="noopener noreferrer">golang backend and react frontend</a>. Board game PDFs are retrieved from https://en.1jour-1jeu.com/rules and stored in R2 if retrieving for the first time

#### try it out
